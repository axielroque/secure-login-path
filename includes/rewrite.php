<?php
defined( 'ABSPATH' ) || exit;

add_action( 'init', 'logkit_register_rewrite_rule', 1 );

function logkit_register_rewrite_rule() {
    $slug = logkit_get_login_slug();
    if ( empty( $slug ) ) return;

    // Rewrite custom login path
    add_rewrite_rule(
        '^' . preg_quote( $slug ) . '/?$',
        'index.php?logkit_login=1',
        'top'
    );
}
add_filter( 'query_vars', 'logkit_add_query_vars' );
function logkit_add_query_vars( $vars ) {
    $vars[] = 'logkit_login';
    return $vars;
}

 

// Note: the login POST can go to wp-login.php; we allow it in logkit_intercept_direct_login()

// Block direct access to wp-login.php before WordPress processes it
add_action( 'init', 'logkit_intercept_direct_login', 1 );
function logkit_intercept_direct_login() {
    // Only act if the request is targeting wp-login.php directly
    $script_name = isset( $_SERVER['SCRIPT_NAME'] ) ? basename( sanitize_text_field( wp_unslash( $_SERVER['SCRIPT_NAME'] ) ) ) : '';
    $request_uri = isset( $_SERVER['REQUEST_URI'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
    $is_login_script = ( $script_name === 'wp-login.php' );
    $is_login_uri = ( $request_uri && strpos( $request_uri, 'wp-login.php' ) !== false );
    if ( ! $is_login_script && ! $is_login_uri ) {
        return;
    }

    // Allow POST requests so authentication can work
    $method = isset( $_SERVER['REQUEST_METHOD'] ) ? strtoupper( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) ) : 'GET';
    if ( $method === 'POST' ) {
        return;
    }

    // Allow only legitimate GET actions
    $action = sanitize_key( (string) filter_input( INPUT_GET, 'action', FILTER_UNSAFE_RAW ) );
    $allowed_actions = [ 'lostpassword', 'rp', 'resetpass', 'register', 'logout', 'postpass', 'verifyemail', 'confirm_admin_email', 'reauth' ];
    $allowed_actions = (array) apply_filters( 'logkit_allowed_login_actions', $allowed_actions, $action );
    if ( in_array( $action, $allowed_actions, true ) ) {
        return;
    }
    // Allow if the referer comes from the custom login path
    $slug = logkit_get_login_slug();
    if ( ! empty( $slug ) ) {
        $referer = isset( $_SERVER['HTTP_REFERER'] ) ? esc_url_raw( wp_unslash( $_SERVER['HTTP_REFERER'] ) ) : '';
        if ( $referer && strpos( $referer, '/' . $slug . '/' ) !== false ) {
            return;
        }
    }
    
    // Allow recovery mode
    if ( logkit_is_recovery_mode() ) {
        return;
    }
    
    $slug = logkit_get_login_slug();
    if ( empty( $slug ) ) return;
    
    // Block according to configured behavior
    logkit_send_block_response();
}

// Mark custom login context early
add_action( 'parse_request', 'logkit_mark_custom_login', 1 );
function logkit_mark_custom_login( $wp ) {
    if ( isset( $wp->query_vars['logkit_login'] ) && $wp->query_vars['logkit_login'] ) {
        define( 'LOGKIT_CUSTOM_LOGIN', true );
    }
}

add_action( 'template_redirect', 'logkit_maybe_render_login' );
function logkit_maybe_render_login() {
    if ( get_query_var( 'logkit_login' ) ) {
        // Initialize variables expected by wp-login.php
        global $error, $interim_login, $action, $user_login;
        $error = '';
        $interim_login = false;
        $action = sanitize_key( (string) filter_input( INPUT_GET, 'action', FILTER_UNSAFE_RAW ) );
        if ( $action === '' ) {
            $action = 'login';
        }
        $user_login = '';
        nocache_headers();
        if ( ! headers_sent() ) {
            header( 'X-Robots-Tag: noindex, nofollow', true );
        }

        require_once ABSPATH . 'wp-login.php';
        exit;
    }
}


